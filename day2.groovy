def pattern = ~'([0-9]+)-([0-9]+) ([a-z]): ([a-z0-9]+)'
parseLine = { line ->
    def matcher = line =~ pattern
    matcher.find()
    [
        num1: matcher.group(1) as Integer,
        num2: matcher.group(2) as Integer,
        character: matcher.group(3),
        password: matcher.group(4)
    ]
}
def isValid = { line -> 
    def record = parseLine(line.trim())
    def count = record.password.count(record.character)
    count in (record.num1..record.num2)
}
​def isValid2 = { line ->
    def record = parseLine(line.trim())
    record.password[record.num1 - 1] == record.character ^ record.password[record.num2 - 1] == record.character
}
...
def part1 = input.collect {
    isValid it
}

def part2 = input.collect {
    isValid2 it
}

"${part1.count(true)} ${part2.count(true)}"​​​​​​​​​​​​​​​​​​​​​​​​
...
2-11 j: sjjrtjkjhjj
6-7 m: mlmrrmm`;
countCharacters = (str, character) =>
    str.split('').filter(x => x == character).length;
isValid = (line) => {
    const [validationRules, password] = line.split(':').map(x => x.trim());
    const [occurances, letter] = validationRules.split(' ');
    const [min, max] = occurances.split('-');
    characterCount = countCharacters(password, letter);
    return characterCount <= max && characterCount >= min;
};
isValid2 = (line) => {
    const [validationRules, password] = line.split(':').map(x => x.trim());
    const [occurances, letter] = validationRules.split(' ');
    const [idx1, idx2] = occurances.split('-');
	return password[+idx1 - 1] == letter ^ password[+idx2 - 1] == letter;
}
inputArray = input.split(/\r?\n/);
console.log(inputArray.map(isValid).filter(x => x).length)
console.log(inputArray.map(isValid2).filter(x => x).length)